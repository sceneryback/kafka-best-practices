...
func (p *Producer) StartProduce(topic string) {
	...
	for i := 0; ; i++ {
		...
		case p.p.Input() <- &sarama.ProducerMessage{
			...
			if i % 5000 == 0 {
				fmt.Printf("produced %d messages with speed %.2f/s", i, float64(i) / time.Since(start).Seconds())
			}
		case err := <-p.p.Errors():
			fmt.Printf("Failed to send message to kafka, err: %s, msg: %s", err, msgBytes)
		case <-p.exitC:
			return
		}
	}
